...
def copy_folders_to_TEST_EXECUTION_FOLDER():
    ...
    copy_the_hull_vidispine_addon_chart_files_to_test_execution_folder(data_store.scenario.case, data_store.scenario.chart)
...
@step("Copy the HULL vidispine addon chart files for test case <case> and chart <chart> to test execution folder")
def copy_the_hull_vidispine_addon_chart_files_to_test_execution_folder(case, chart):
    dir_path = os.path.dirname(os.path.realpath(__file__))
    hull_path = os.path.join(dir_path,'./../../../../')
    dst_path = os.path.join(dir_path, TEST_EXECUTION_FOLDER, 'case', case, 'chart', chart, "charts/hull-vidispine-addon-1.0.0/")
    try:
        ...
        copyfile(hull_path, "hull-vidispine-addon.yaml", os.path.join(dir_path, TEST_EXECUTION_FOLDER, 'case', case, 'chart', chart))        
    except Exception as e:
        print("Oops!", e.__str__, "occurred.")
        assert False
    assert True    
...
@step("Test Object has key <key> with non empty value")
def test_object_has_key_with_non_empty_value(key):
    ...
    assert_values_not_equal(data_store.scenario.test_object[key], '', key)
...
def assert_values_not_equal(actual, not_expected, key=None):
    assert not_expected != actual, "For key '"+ key + "' there was not expected value:\n\n" + str(not_expected) + "\n\nfound"
